...
if(isset($_POST['supprimer']))      //supression des catégorie
{
    ...
    $sql="SELECT cat_id,cat_autorisation from t_categorie_cat where cat_id ='".$id."'";
    $res=$mysqli->query($sql);
    //echo $sql;

    if($cat['cat_autorisation']=='R'){
        if($res==false)
        {
            echo "ERREUR : $sql";
        }
...
if(isset($_POST['modifier'])){
    ...
    //chercher l'actualité avec cet id
    $sql="SELECT cat_autorisation from t_categorie_cat where cat_id =".$_GET['edit']."";
    $res=$mysqli->query($sql);
    $cat=$res->fetch_assoc();
    ...
    header("Location: red_cat_edit.php?edit=".$_GET['edit']."");
}
